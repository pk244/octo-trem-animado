...
mod test {
    ...
    async fn test_testnet_upgrade_compatibility() {
        ...
        let mut test_cluster = init_test_cluster_builder(7, 3000)
        ...
        // It is known that protocol v1 has a bug that could lead to safe mode.
        ...
        let _handle = tokio::task::spawn(async move {
            for version in min_ver..max_ver {
                ...
                test_cluster.all_node_handles().for_each(|h| {
                    h.with(|node| {
                        assert_eq!(
                            node.state()
                ...
                let new_framework_modules: Vec<_> = match new_framework {
                    Ok(new_framework) => new_framework
                        .into_iter()
                        .flat_map(|object| {
                            object
                                .data
                    ...
                    Err(_) => {
                        ...
                        sui_framework::make_system_modules()
                            .into_iter()
                            .flatten()
                            .collect()
                    }
                };
                sui_system_injection::set_override(new_framework_modules);
                ...
                while test_cluster.wait_for_epoch(None).await.protocol_version() == 1 {}
