...
const GIT_REVISION: &str = git_version!(args = ["--dirty="]);

fn main() {
    let network = parse_network_from_args();
    ...
    update_manifest(&network, version, files);
}

fn parse_network_from_args() -> String {
    let args: Vec<String> = env::args().collect();
    if args.len() != 2 {
        eprintln!("Usage: {} <devnet|testnet|mainnet>", args[0]);
        std::process::exit(1);
    }
...
fn update_manifest(network: &str, version: u64, files: Vec<String>) {
    // Remove all prefixes and only keep the git hash.
    // "git describe" always prefix git hash with "g".
    let git_revision = GIT_REVISION
        .split_at(GIT_REVISION.rfind("-g").unwrap() + 2)
        .1
        .to_string();
